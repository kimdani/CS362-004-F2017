...
int playAdventurer(struct gameState* state)
{
  ...
  while(drawntreasure<2){
    ...
    if (cardDrawn == copper) // BUG introduced here by removing the || and following arguments in this if statement
      drawntreasure++;
    else{
...
int playSmithy(struct gameState* state, int handPos)
{
  ...
  //+3 Cards
  for (i = 0; i < 4; i++) // BUG introduced here by allowing the player to draw 4 cards instead of 3 by playing smithy.
  {
    drawCard(currentPlayer, state);
  }
...
{ 
  int currentPlayer = whoseTurn(state);
    ...
    //+2 Actions
    state->numActions = state->numActions + 1; // BUG introduced by increasing player's actions for this turn by 1 instead of 2.
...
int playCouncilRoom(struct gameState* state, int handPos)
{
  ...
  //  BUG INTRODUCED HERE: No players draw extra cards when the Council Room card is played.
